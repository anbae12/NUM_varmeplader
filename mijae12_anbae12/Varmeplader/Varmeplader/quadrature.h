...
struct Trapzd : Quadrature {
    ...
    Doub next() {
        ...
        if (n == 1) {
            ...
            std::cout<<n<<std::setw(setww)<<s<<std::setw(setww)<<0<<std::setw(setww)<<0<<std::endl;
            return s;
        } else {
            s_last_last = s_last;
            s_last = s;
            ...
            if(n>2){
                std::cout<<n<<std::setw(setww)<<s<<std::setw(setww)<<log((s_last_last-s_last)/(s_last-s))/log(3)<<std::setw(setww)<<(s-s_last)/((s_last_last-s_last)/(s_last-s)-1)<<std::endl;
            }
            else {
                std::cout<<n<<std::setw(setww)<<s<<std::setw(setww)<<0<<std::setw(setww)<<0<<std::endl;
            }
